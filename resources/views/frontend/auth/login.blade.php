...
<section style="padding-top: 160px; padding-bottom: 90px; min-height: 85vh; background: var(--bg); display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden;">
  ...
  <div class="login-card" style="background: var(--bg-card); border: 1px solid var(--border); border-radius: 8px; width: 100%; max-width: 440px; padding: 40px; box-shadow: 0 15px 35px rgba(0,0,0,0.6); z-index: 10; font-family: 'Jost', sans-serif; box-sizing: border-box;">
    ...
    <!-- Alert Messages -->
    @if(session('error'))
      <div style="background: rgba(192, 57, 43, 0.1); border: 1px solid var(--red); color: var(--red2); padding: 12px; border-radius: 4px; margin-bottom: 20px; font-size: 13.5px;">
        {{ session('error') }}
      </div>
    @endif
    @if(session('success'))
      <div style="background: rgba(39, 174, 96, 0.1); border: 1px solid var(--green); color: var(--green); padding: 12px; border-radius: 4px; margin-bottom: 20px; font-size: 13.5px;">
        {{ session('success') }}
      </div>
    @endif
    ...
    <form action="{{ route('login.submit') }}" method="POST">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; font-size: 13px; color: var(--text-light);">
        <label style="display: flex; align-items: center; cursor: pointer;">
          <input type="checkbox" name="remember" style="margin-right: 6px; accent-color: var(--gold);"> Remember Me
        </label>
      </div>
      ...
      <div style="text-align: center; margin-top: 25px; font-size: 13.5px; color: var(--text-light);">
        ...
        <a href="{{ route('register') }}" style="color: var(--gold); text-decoration: none; font-weight: 600; margin-left: 4px;" onmouseover="this.style.textDecoration='underline';" onmouseout="this.style.textDecoration='none';">Create Account</a>
      </div>
