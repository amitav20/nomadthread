...
@section('title', 'Edit Page | LeatherCraft')
@section('page_title', 'Edit Page')
@section('page_breadcrumb', 'Website / Edit Page')
...
<form action="{{ route('backend.pages.update', $page->id) }}" method="POST" enctype="multipart/form-data" onsubmit="document.getElementById('pageContentInput').value = document.querySelector('.content-editor').innerHTML;">
  @csrf
  @method('PUT')
  <input type="hidden" name="content" id="pageContentInput">

  <div class="page-header">
    <div><div class="page-heading">Edit Page: {{ $page->title }}</div><div class="page-subheading">Update content details and SEO settings for this page</div></div>
    <div class="page-actions">
      <a href="{{ route('backend.pages.index') }}" class="btn btn-outline"><i class="ti ti-x"></i> Cancel</a>
      <button type="submit" class="btn btn-gold"><i class="ti ti-device-floppy"></i> Update Page</button>
    </div>
  </div>
  ...
  <div class="form-layout">
    <div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="form-group">
            ...
            <input type="text" name="title" class="form-input" id="pageNameInput" placeholder="e.g. About Us — LeatherCraft India" value="{{ old('title', $page->title) }}" oninput="updatePageSlug()" required>
          </div>
          <div class="form-group">
            ...
            <div style="display:flex;gap:0;">
              ...
              <input type="text" name="slug" class="form-input" style="border-radius:0 8px 8px 0;" id="pageSlugInput" placeholder="about-us" value="{{ old('slug', $page->slug) }}" required>
            </div>
          </div>
          <div class="form-group" style="margin-bottom:0;">
            <label class="form-label">Page Type</label>
            <select name="page_type" class="form-input">
              <option value="Custom Page" {{ old('page_type', $page->page_type) == 'Custom Page' ? 'selected' : '' }}>Custom Page</option>
              <option value="About Us" {{ old('page_type', $page->page_type) == 'About Us' ? 'selected' : '' }}>About Us</option>
              <option value="Contact Us" {{ old('page_type', $page->page_type) == 'Contact Us' ? 'selected' : '' }}>Contact Us</option>
              <option value="Privacy Policy" {{ old('page_type', $page->page_type) == 'Privacy Policy' ? 'selected' : '' }}>Privacy Policy</option>
              <option value="Return & Refund Policy" {{ old('page_type', $page->page_type) == 'Return & Refund Policy' ? 'selected' : '' }}>Return & Refund Policy</option>
              <option value="Shipping Policy" {{ old('page_type', $page->page_type) == 'Shipping Policy' ? 'selected' : '' }}>Shipping Policy</option>
              <option value="Terms & Conditions" {{ old('page_type', $page->page_type) == 'Terms & Conditions' ? 'selected' : '' }}>Terms & Conditions</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="content-editor" contenteditable="true" data-placeholder="Start writing your page content here…" style="min-height:280px;">{!! old('content', $page->content) !!}</div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="form-group" style="margin-bottom:0;">
            <label class="form-label">Featured / OG Image (Replaces current)</label>
            <div class="upload-drop" style="position:relative;">
              ...
              <p>Upload new featured page header image</p>
              <span>Recommended: 1200×630px</span>
            </div>
            
            @if(!empty($page->featured_image))
              <div style="margin-top: 15px;">
                <p style="font-size:12px; color:var(--text2); margin-bottom: 6px;">Current Featured Image:</p>
                <img src="{{ $page->featured_image }}" style="max-width:150px; border-radius:6px; border:1px solid var(--border);" alt="featured image">
              </div>
            @endif
          </div>
        </div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <input type="datetime-local" name="schedule_publish" class="form-input" value="{{ old('schedule_publish', $page->schedule_publish ? date('Y-m-d\TH:i', strtotime($page->schedule_publish)) : '') }}">
          </div>
          <div class="divider"></div>
          <div class="form-group">
            <div class="toggle-group">
              <div class="toggle-info"><p>Show in Navigation</p><span>Add to site menu</span></div>
              <label class="toggle"><input type="checkbox" name="show_in_navigation" value="1" {{ $page->show_in_navigation ? 'checked' : '' }}><span class="toggle-slider"></span></label>
            </div>
            <div class="toggle-group">
              <div class="toggle-info"><p>Show in Footer</p><span>Add to footer links</span></div>
              <label class="toggle"><input type="checkbox" name="show_in_footer" value="1" {{ $page->show_in_footer ? 'checked' : '' }}><span class="toggle-slider"></span></label>
            </div>
            <div class="toggle-group">
              <div class="toggle-info"><p>Index by Search Engines</p><span>Allow Google to crawl this page</span></div>
              <label class="toggle"><input type="checkbox" name="index_by_search_engines" value="1" {{ $page->index_by_search_engines ? 'checked' : '' }}><span class="toggle-slider"></span></label>
            </div>
          </div>
          <div class="divider"></div>
          <button type="submit" class="btn btn-gold btn-block btn-lg"><i class="ti ti-device-floppy"></i> Update Page</button>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="form-group">
            ...
            <input type="text" name="meta_title" class="form-input" id="metaTitlePage" placeholder="Page Title | LeatherCraft" value="{{ old('meta_title', $page->meta_title) }}" oninput="updateSeoPage()">
            <div class="form-hint" id="pageTitleCount">0 / 60 characters</div>
          </div>
          <div class="form-group">
            <label class="form-label">Meta Description</label>
            <textarea name="meta_description" class="form-input" style="min-height:75px;" id="metaDescPage" placeholder="Brief page description for search engines…" oninput="updateSeoPage()">{{ old('meta_description', $page->meta_description) }}</textarea>
            <div class="form-hint" id="pageDescCount">0 / 160 characters</div>
          </div>
          <div class="form-group">
            <label class="form-label">Focus Keyword</label>
            <input type="text" name="focus_keyword" class="form-input" placeholder="e.g. leather bags india" value="{{ old('focus_keyword', $page->focus_keyword) }}">
          </div>
          <div class="form-group" style="margin-bottom:0;">
            <label class="form-label">Search Preview</label>
            <div class="seo-preview">
              <div class="seo-url" id="seoPageUrl">https://leathercraft.in/pages/{{ $page->slug }}</div>
              <div class="seo-title" id="seoPageTitle">{{ $page->meta_title ?: $page->title }} | LeatherCraft India</div>
              <div class="seo-desc" id="seoPageDesc">{{ $page->meta_description ?: 'Your meta description will appear here…' }}</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card-body">
          <div style="display:flex;flex-direction:column;gap:8px;">
            <label style="display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid var(--border);border-radius:8px;cursor:pointer;">
              <input type="radio" name="template" value="Default Page" {{ $page->template == 'Default Page' ? 'checked' : '' }} style="accent-color:var(--gold);">
            ...
            <label style="display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid var(--border);border-radius:8px;cursor:pointer;">
              <input type="radio" name="template" value="With Sidebar" {{ $page->template == 'With Sidebar' ? 'checked' : '' }} style="accent-color:var(--gold);">
            ...
            <label style="display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid var(--border);border-radius:8px;cursor:pointer;">
              <input type="radio" name="template" value="Contact Page" {{ $page->template == 'Contact Page' ? 'checked' : '' }} style="accent-color:var(--gold);">
            ...
            <label style="display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid var(--border);border-radius:8px;cursor:pointer;">
              <input type="radio" name="template" value="Policy Page" {{ $page->template == 'Policy Page' ? 'checked' : '' }} style="accent-color:var(--gold);">
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    if (window.updateSeoPage) {
      window.updateSeoPage();
    }
  });
</script>
@endsection
