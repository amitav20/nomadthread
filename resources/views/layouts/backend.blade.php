...
<body class="flex h-full overflow-hidden bg-slate-950">
    ...
    <aside class="w-64 sidebar-glass flex flex-col justify-between shrink-0">
        <div>
            ...
            <div class="h-16 flex items-center px-6 border-b border-slate-800/80">
                <a href="{{ route('backend.dashboard') }}" id="sidebar-logo" class="flex items-center gap-2">
                    <span class="bg-gradient-to-r from-teal-400 to-cyan-400 bg-clip-text text-xl font-bold tracking-widest text-transparent uppercase">
                        Admin Portal
                    </span>
                </a>
            </div>
            ...
            <nav class="p-4 space-y-1.5">
                <a href="{{ route('backend.dashboard') }}" id="admin-nav-dashboard" class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all {{ Route::is('backend.dashboard') ? 'bg-teal-500/10 text-teal-300 border-l-2 border-teal-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200' }}">
                ...
                <a href="{{ route('backend.users.index') }}" id="admin-nav-users" class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all {{ Route::is('backend.users.*') ? 'bg-teal-500/10 text-teal-300 border-l-2 border-teal-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                ...
                <a href="{{ route('backend.threads.index') }}" id="admin-nav-threads" class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all {{ Route::is('backend.threads.*') ? 'bg-teal-500/10 text-teal-300 border-l-2 border-teal-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
        ...
        <div class="p-4 border-t border-slate-800/80">
            <a href="{{ route('home') }}" id="admin-nav-exit" class="flex w-full items-center justify-center gap-2 rounded-lg bg-slate-800 py-2 text-xs font-semibold text-slate-300 border border-slate-700/60 hover:bg-slate-700 hover:text-white transition-all duration-300">
    ...
    <div class="flex-grow flex flex-col overflow-hidden">
        ...
        <!-- Top bar -->
        <header class="h-16 bg-slate-900 border-b border-slate-800/80 flex items-center justify-between px-8 shrink-0">
            <div>
